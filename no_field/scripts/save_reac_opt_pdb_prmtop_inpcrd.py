...
opt_geoms = sorted((glob.glob('../rep_1*/a_reac/dna_r_opt.pdb')))
...
#opt_geoms = sorted(pick_include_set(glob.glob('rep_1*/a_reac/reac_opt.pdb'), inc_list))
#prmtops   = sorted(pick_include_set(glob.glob('rep_1*/initial_struc/reac.prmtop'), inc_list))
#print(len(opt_geoms), len(prmtops))
...
for i in np.arange(len(prmtops)):
    ...
    dna.save(f'../{rep_string}/optimised_struc/dna_r_opt.pdb', overwrite=True)
    dna.save(f'../{rep_string}/optimised_struc/dna_r_opt.prmtop', overwrite=True)
    dna.save(f'../{rep_string}/optimised_struc/dna_r_opt.inpcrd', overwrite=True)
