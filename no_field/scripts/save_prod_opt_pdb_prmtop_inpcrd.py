...
opt_geoms = sorted((glob.glob('../rep_1*/e_product/dna_p_opt.pdb')))
...
for i in np.arange(len(prmtops)):
    ...
    dna.save(f'../{rep_string}/optimised_struc/dna_p_opt.pdb', overwrite=True)
    dna.save(f'../{rep_string}/optimised_struc/dna_p_opt.prmtop', overwrite=True)
    dna.save(f'../{rep_string}/optimised_struc/dna_p_opt.inpcrd', overwrite=True)
