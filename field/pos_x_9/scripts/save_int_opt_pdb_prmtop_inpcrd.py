...
opt_geoms = sorted(glob.glob('../rep_1*/c_int/int_opt.pdb'))
...
for i in np.arange(len(prmtops)):
	    ...
	    dna.save(f'../{rep_string}/optimised_struc/int_opt.pdb', overwrite=True)
	    dna.save(f'../{rep_string}/optimised_struc/int_opt.prmtop', overwrite=True)
	    dna.save(f'../{rep_string}/optimised_struc/int_opt.inpcrd', overwrite=True)
        print( 'Saved optimised Intermediate files for '+ rep_string)
