...
opt_geoms = sorted(glob.glob('../rep_1*/d_ts2/ts2_dimer.pdb'))
...
for i in np.arange(len(prmtops)):
	    ...
	    dna.save(f'../{rep_string}/optimised_struc/ts2_opt.pdb', overwrite=True)
	    dna.save(f'../{rep_string}/optimised_struc/ts2_opt.prmtop', overwrite=True)
	    dna.save(f'../{rep_string}/optimised_struc/ts2_opt.inpcrd', overwrite=True)
        print( 'Saved optimised TS2 files for '+ rep_string)
