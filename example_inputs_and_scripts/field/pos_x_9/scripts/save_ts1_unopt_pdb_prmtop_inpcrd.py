...
opt_geoms = sorted(pick_include_set(glob.glob('rep_1*/neb/ts1.pdb'),include))
...
if len(opt_geoms)==len(prmtops):
	for i in np.arange(len(prmtops)):
	    ...
	    dna.save(f'{rep_string}/initial_struc/ts1.pdb', overwrite=True)
	    dna.save(f'{rep_string}/initial_struc/ts1.prmtop', overwrite=True)
	    dna.save(f'{rep_string}/initial_struc/ts1.inpcrd', overwrite=True)   
	    print(rep_string)

print( 'Saved initial TS1 files for '+ rep_string)
